          ...
          <Tabs value={tab} onValueChange={(v) => { setTab(v); setError(''); }}>
            ...
            <TabsContent value="register">
              <form onSubmit={handleRegister} className="space-y-5" data-testid="register-form">
                ...
                <div className="space-y-2">
                  <Label className="text-sm text-foreground">Giriş kodu *</Label>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                    <Input
                      data-testid="register-admin-secret"
                      type="password"
                      value={regForm.admin_secret}
                      onChange={(e) => setRegForm({ ...regForm, admin_secret: e.target.value })}
                      placeholder="Admin giriş kodunu daxil edin"
                      className="pl-10 bg-white/5 border-white/10 focus:border-primary h-12 rounded-lg"
                    />
                  </div>
                </div>
